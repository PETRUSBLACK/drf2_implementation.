...
def verify_token(token):
    # decode the token
    try:
        decoded_data = jwt.decode(token, settings.SECRET_KEY, slgorithm="HS256")
    except Exception:
        return None
    
    # check if token has expirted
...
class RefreshView(APIView):
    ...
    def post(self, request):
        ...
        if not verify_token(serializer.validata_data["refresh"]):
